...
from pycbc.workflow.core import Executable,FileList,Node,File,make_analysis_dir
import sys
import h5py
...
class PyCBCBank2HDFExecutable(Executable):
    ...
    def create_node(self, bank_file):
        ...
        node.new_output_file_opt(bank_file.segment, 'stage2.hdf', '--output-file')
        return node
...
class PyCBCTrig2HDFExecutable(Executable):
    ...
    def create_node(self, trig_files, bank_file):
        ...
        node.new_output_file_opt(trig_files[0].segment, 'stage2.hdf',
...
class PyCBCFitByTemplateExecutable(Executable):
    ...
    def create_node(self, trig_file, bank_file, veto_file, veto_name):
        ...
        node.new_output_file_opt(trig_file.segment, 'stage2.hdf', '--output')
        return node

class PyCBCFitOverParamExecutable(Executable):
    """Smooths the background distribution parameters over a continuous parameter"""
    current_retention_level = Executable.MERGED_TRIGGERS
    def create_node(self, trig_file, bank_file, veto_file,veto_name,fit_stage1,template_str):
        node = Node(self)
        node.add_opt('--ifo', self.ifo_string)
        ...
        node.add_opt('--veto-segment-name', veto_name)
        node.add_input_opt('--bank-file', bank_file)
        node.add_opt('--fit-stage1',fit_stage1)
        node.new_output_file_opt(trig_file.segment,'stage2.hdf', '--output',tags=[str(int(template_str.split('/')[0]))])
        #node.new_output_file_opt(raw_fit_file.segment, 'stage2.hdf', '--output')
        return node
...
class PyCBCFindCoincExecutable(Executable):
    ...
    def create_node(self, trig_files, bank_file, stat_files, veto_file,
        ...
        node.add_opt('--template-fraction-range', template_str)
        node.new_output_file_opt(seg, 'stage2.hdf', '--output-file', tags=tags)
        return node
...
class PyCBCHDFInjFindExecutable(Executable):
    ...
    def create_node(self, inj_coinc_file, inj_xml_file, veto_file, veto_name, tags=None):
        ...
        if veto_name is not None:
            node.add_input_opt('--veto-file', veto_file)
        node.add_opt('--segment-name', veto_name)
        node.new_output_file_opt(inj_xml_file[0].segment, 'stage2.hdf', '--output-file',
                                 tags=tags)
        return node
...
class PyCBCDistributeBackgroundBins(Executable):
    ...
    def create_node(self, coinc_files, bank_file, background_bins, tags=None):
        ...
        output_files = [File(coinc_files[0].ifo_list,
                             ...
                             tags = tags + ['mbin-%s' % i],
                             extension='stage2.hdf') for i in range(len(background_bins))]
...
class PyCBCCombineStatmap(Executable):
    ...
    def create_node(self, statmap_files, tags=None):
        ...
        node.new_output_file_opt(statmap_files[0].segment, 'stage2.hdf',
...
class PyCBCMultiifoAddStatmap(PyCBCMultiifoCombineStatmap):
    ...
    def create_node(self, statmap_files, background_files, tags=None):
        ...
        if 'INJECTIONS' in ctags: #if 'injections' in tags:
...
class PyCBCMultiifoExcludeZerolag(Executable):
    ...
    def create_node(self, statmap_file, other_statmap_files, tags=None):
        ...
        node.add_input_list_opt('--other-statmap-files',
                                other_statmap_files)
        node.new_output_file_opt(statmap_file.segment, 'stage2.hdf',
...
def setup_trigger_fitting(workflow, insps, hdfbank, veto_file, veto_name,fitstage1,tags=None):   
    if tags is None:
        tags = []  
    if not workflow.cp.has_option('workflow-coincidence', 'do-trigger-fitting'):
        ...
        factor = int(workflow.cp.get_opt_tags('workflow-coincidence-full',
            ...
            for i in workflow.ifos:
                ...
                if i==str('H1'):
                    smooth_node = PyCBCFitOverParamExecutable(workflow.cp,
                        'fit_over_param', ifos=i).create_node(ifo_insp[0],hdfbank[0],veto_file[0],
                                                              veto_name[0],fitstage1[0],group_str)
                else:
                    smooth_node = PyCBCFitOverParamExecutable(workflow.cp,
                        'fit_over_param', ifos=i).create_node(ifo_insp[0],hdfbank[0],veto_file[0],
                                                              veto_name[0],fitstage1[1],group_str)
                workflow += smooth_node
                smoothed_fit_files += smooth_node.output_files
            
        return smoothed_fit_files

class FITMERGEExecutable(Executable):
    current_retention_level = 3
    
    
def combine_fit_param_func(workflow, fit_files, out_dir, tags=None):
    ...
    for ifo in workflow.ifos:
        ...
        node = FITMERGEExecutable(workflow.cp, 'fit_over_param_combine',
                        ifos=ifo, out_dir=out_dir, tags=tags).create_node()
        
        node.add_input_list_opt('--fit-files', fit_files.find_output_with_ifo(ifo))
        node.new_output_file_opt(workflow.analysis_time,'.hdf', '--output-file')
        workflow += node
        out += node.output_files
    ...
    for trig_files, ctag in combo:
        ...
        for i in range(factor):
            ...
            coinc_node = findcoinc_exe.create_node(trig_files, hdfbank,
                                                   ...
                                                   pivot_ifo,
                                                   fixed_ifo,
                                                   tags=['JOB'+str(i)])#tags=[veto_name, str(i)])
...
def setup_multiifo_combine_statmap(workflow, final_bg_file_list, bg_file_list,
    ...
    if cstat_exe_name == 'pycbc_multiifo_combine_statmap':
        combine_statmap_node = cstat_exe.create_node(final_bg_file_list)#,tags=tags)
    elif cstat_exe_name == 'pycbc_multiifo_add_statmap':
        combine_statmap_node = cstat_exe.create_node(final_bg_file_list,
                                                     bg_file_list)#,tags=tags)
...
def rerank_coinc_followup(workflow, statmap_file, bank_file, out_dir, tags=None,
                          injection_file=None,
                          ranking_file=None):
    ...
    for ifo in workflow.ifos:
        ...
        fil = node.new_output_file_opt(workflow.analysis_time, 'stage2.hdf',
                                       '--output-file')#, tags=tags)
        stores.append(fil)
        workflow += node
    ...
    node.add_input_opt('--statmap-file', statmap_file)
    node.add_input_opt('--bank-file', bank_file)
    trigfil = node.new_output_file_opt(workflow.analysis_time, 'stage2.hdf',
                                   '--output-file')#, tags=tags)
    workflow += node
    ...
    for i in range(factor):
        ...
        node.new_output_file_opt(workflow.analysis_time, 'stage2.hdf',
    ...
    node.new_output_file_opt(workflow.analysis_time, 'stage2.hdf',
                             '--output-file')
    workflow += node
    return node.output_file#node.output_files[0]

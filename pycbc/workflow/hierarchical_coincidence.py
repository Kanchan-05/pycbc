...
from pycbc.workflow.core import Executable, FileList, make_analysis_dir 
from hierarchical_core import Node, File
...
def merge_single_detector_hdf_files(workflow, bank_file, trigger_files, out_dir, tags=None):
    ...
    for ifo in workflow.ifos:
        ...
        node.new_output_file_opt(workflow.analysis_time,'stage2.hdf', '--output-file')
        workflow += node
            ...
            for i in workflow.ifos:
                ...
                if i==str('H1'):
                    smooth_node = PyCBCFitOverParamExecutable(workflow.cp,
                        'fit_over_param', ifos=i).create_node(ifo_insp,hdfbank,veto_file,
                                                              veto_name,fitstage1[0],group_str)
                else:
                    smooth_node = PyCBCFitOverParamExecutable(workflow.cp,
                        'fit_over_param', ifos=i).create_node(ifo_insp,hdfbank,veto_file,
                                                              veto_name,fitstage1[1],group_str)
